...
import { ShoppingCart, Search, Edit2, Eye, ArrowLeft, Clock, CheckCircle, AlertCircle, Plus, Trash2 } from 'lucide-react';
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-96 overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <strong>Email:</strong> {viewingOrder.customerId?.email || 'N/A'}
                </div>
                <div>
                ...
                <div>
                  <strong>Unit Price:</strong> {new Intl.NumberFormat('en-KE', {
                    style: 'currency',
                    currency: 'KES'
                  }).format(viewingOrder.items?.[0]?.unitPrice || 0)}
                </div>
                <div>
                ...
                <div>
                  <strong>Status:</strong> <span className={`px-2 py-1 rounded-full text-xs ${getStatusBadgeColor(viewingOrder.status)}`}>
                    {viewingOrder.status === 'in_progress' ? 'In Progress' : viewingOrder.status}
                  </span>
                </div>
                <div>
                  <strong>Payment Status:</strong> <span className={`font-medium ${getPaymentStatusColor(viewingOrder.paymentStatus)}`}>
                    {viewingOrder.paymentStatus || 'pending'}
                  </span>
                </div>
                <div>
                  <strong>Created Date:</strong> {viewingOrder.creationDate ? new Date(viewingOrder.creationDate).toLocaleDateString() : 
                    (viewingOrder.createdAt ? new Date(viewingOrder.createdAt).toLocaleDateString() : 'N/A')}
                </div>
                <div>
                  <strong>Assigned To:</strong> {viewingOrder.assignedTo?.fullName || viewingOrder.assignedTo?.firstName + ' ' + viewingOrder.assignedTo?.lastName || 'N/A'}
                </div>
                <div>
                  <strong>Created By:</strong> {viewingOrder.createdBy?.fullName || viewingOrder.createdBy?.firstName + ' ' + viewingOrder.createdBy?.lastName || 'N/A'}
                </div>
                {viewingOrder.notes && (
                  <div>
